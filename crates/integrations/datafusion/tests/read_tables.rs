...
async fn test_time_travel_by_timestamp() {
    // Read snapshot-1's commit time, then query with a timestamp between snapshot-1
    // and snapshot-2 to verify timestamp-based time travel returns snapshot-1's data.
    let warehouse = get_test_warehouse();
    let table_path = format!("{}/default.db/time_travel_table", warehouse);
    let snap1_path = format!("{}/snapshot/snapshot-1", table_path);
    let snap1_bytes = std::fs::read(&snap1_path).expect("snapshot-1 should exist");
    let snap1: serde_json::Value =
        serde_json::from_slice(&snap1_bytes).expect("snapshot-1 should be valid JSON");
    let snap1_millis = snap1["timeMillis"]
        .as_u64()
        .expect("timeMillis should exist") as i64;

    // Format as local timestamp string (matches parse_timestamp_to_millis which uses Local).
    let snap1_ts = chrono::Local
        .timestamp_millis_opt(snap1_millis)
        .single()
        .expect("valid timestamp");
    let ts_str = snap1_ts.format("%Y-%m-%d %H:%M:%S").to_string();

    let ctx = create_time_travel_context().await;
    let query = format!(
        "SELECT id, name FROM paimon.default.time_travel_table FOR SYSTEM_TIME AS OF '{ts_str}'"
    );
    let batches = ctx
        .sql(&query)
        .await
        .expect("timestamp time travel query should parse")
        .collect()
        .await
        .expect("timestamp time travel query should execute");
    ...
    assert_eq!(
        rows,
        vec![(1, "alice".to_string()), (2, "bob".to_string())],
        "Querying at snapshot-1's commit time should return snapshot-1 data"
    );
}
