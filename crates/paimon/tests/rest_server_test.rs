...
async fn test_drop_database() {
    ...
    // Drop database
    let result = ctx.api.drop_database("to_drop", false).await;
...
#[tokio::test]
async fn test_drop_database_cascade() {
    let ctx = setup_test_server(vec!["default"]).await;

    // Add a table to the database
    ctx.server.add_table("default", "test_table");

    // Drop with cascade
    let result = ctx.api.drop_database("default", true).await;
    assert!(result.is_ok(), "failed to drop database with cascade: {:?}", result);

    // Verify both database and its tables are gone
    let dbs = ctx.api.list_databases().await.unwrap();
    assert!(!dbs.contains(&"default".to_string()));
}
...
async fn test_drop_database_no_permission() {
    ...
    let result = ctx.api.drop_database("secret", false).await;
    assert!(
        result.is_err(),
...
async fn test_create_table() {
    ...
    let schema = Schema::builder()
        ...
        .column("name", DataType::VarChar(paimon::spec::VarCharType::new(255).unwrap()))
        .build()
...
async fn test_drop_table_no_permission() {
    let ctx = setup_test_server(vec!["default"]).await;
    ctx.server.add_no_permission_table("default", "secret_table");

    let result = ctx.api.drop_table("default", "secret_table").await;
    assert!(
        result.is_err(),
        "dropping no-permission table should fail"
    );
}
