...
impl TableWrite {
    ...
    /// `_ROW_ID: Int64`; primary-key writes may append `_VALUE_KIND: Int8` when
    /// the row kind is not derived from a `rowkind.field` or generated by a
    /// cross-partition writer. Any other shape — reordered, renamed,
    ...
    fn validate_write_batch_schema(&self, batch: &RecordBatch) -> Result<()> {
        ...
        // Primary-key writers consume caller-supplied `_VALUE_KIND` regardless
        // of whether a separate changelog file is produced. For rowkind.field
        // and cross-partition writers it is generated internally, so an
        // incoming `_VALUE_KIND` column is illegal there.
        let allow_value_kind = !self.primary_key_indices.is_empty()
            ...
            .is_some_and(|field| field.name() == VALUE_KIND_FIELD_NAME)
        {
            ...
            if !allow_value_kind || value_kind_field.data_type() != &ArrowDataType::Int8 {
                return Err(crate::Error::DataInvalid {
                    message: format!(
                        "write batch schema mismatch at column {suffix_idx}: trailing '{VALUE_KIND_FIELD_NAME}' is only allowed for primary-key writes without rowkind.field or cross-partition updates, with Arrow type Int8, but got {:?}",
...
mod tests {
    ...
    /// Primary-key writers consume caller-supplied row kinds independently of
    /// whether a separate changelog file is produced.
    #[tokio::test]
    async fn test_write_accepts_value_kind_on_primary_key_table() {
        let file_io = test_file_io();
        let table_path = "memory:/test_validate_pk_value_kind";
        setup_dirs(&file_io, table_path).await;
        let table = test_pk_table(&file_io, table_path);
    ...
    /// Append-only writers do not consume caller-supplied `_VALUE_KIND`.
    #[tokio::test]
    async fn test_write_rejects_value_kind_on_append_table() {
        let file_io = test_file_io();
        let table_path = "memory:/test_validate_append_value_kind";
        setup_dirs(&file_io, table_path).await;
        let table = test_table(&file_io, table_path);
