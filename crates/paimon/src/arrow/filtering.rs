...
use crate::spec::{
    field_idx_to_partition_idx, DataField, DataType, Datum, Predicate, PredicateOperator,
};
...
pub(crate) fn reader_pruning_predicates(
    filter: Predicate,
    fields: &[DataField],
    partition_keys: &[String],
) -> Vec<Predicate> {
    let partition_mapping = field_idx_to_partition_idx(fields, partition_keys);
    filter
        .split_and()
        .into_iter()
        .filter(|predicate| {
            !predicate_contains_partition_field(predicate, &partition_mapping)
                && predicate_supported_for_reader_pruning(predicate)
        })
        .collect()
}
...
fn predicate_contains_partition_field(
    predicate: &Predicate,
    partition_mapping: &[Option<usize>],
) -> bool {
    match predicate {
        Predicate::Leaf { index, .. } => partition_mapping.get(*index).is_some_and(Option::is_some),
        Predicate::And(children) | Predicate::Or(children) => children
            .iter()
            .any(|child| predicate_contains_partition_field(child, partition_mapping)),
        Predicate::Not(inner) => predicate_contains_partition_field(inner, partition_mapping),
...
fn data_leaf_may_match<T: StatsAccessor>(
    index: usize,
