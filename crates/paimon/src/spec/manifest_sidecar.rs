...
mod tests {
    ...
    #[test]
    fn java_generated_v1_golden_is_bidirectionally_compatible() {
        // Generated by ManifestSidecarTest.testSidecar() in Apache Paimon Java
        // commit d3e178e29726b5f6ef1ad6d989dc7de684a09374. Keep this as an opaque
        // Java-writer artifact; regenerate it instead of editing it by hand.
        let golden = include_bytes!("goldens/manifest_sidecar_v1.bin");
        let header = header();
        ...
        assert_eq!(
            sidecar.as_slice(),
            golden,
            "Rust V1 output differs from the Java-generated golden fixture"
        );
        assert_eq!(
            u32::from_be_bytes(golden[golden.len() - 4..].try_into().unwrap()),
            0xdf82_cd30
        );
        let manifest = meta(header.len() as u64 + 400, 7);
        assert_eq!(
            ManifestSidecar::select(golden, &manifest, Some(&[RowRange::new(20, 20)]),)
                .unwrap()
                .blocks()
                .iter()
        ...
        let mut partition_miss = |bytes: &[u8]| bytes == b"not-a-partition";
        assert!(select_with_filters(
            golden,
            &manifest,
            None,
            Some(&mut partition_miss),
            Some(2),
            None,
        )
        .unwrap()
        ...
        let mut bucket = |bucket: i32, total: i32| bucket == 2 && total == 8;
        assert_eq!(
            select_with_filters(golden, &manifest, None, None, None, Some(&mut bucket))
                .unwrap()
                .blocks()
                .iter()
                .map(ManifestSidecarBlock::first_record)
