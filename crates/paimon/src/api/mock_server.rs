...
use super::api_request::{AlterDatabaseRequest, RenameTableRequest};
use super::api_response::{
    ConfigResponse, ErrorResponse, GetDatabaseResponse, GetTableResponse, ListDatabasesResponse,
    ListTablesResponse,
};
...
impl RESTServer {
    ...
    pub fn new(
        ...
        // Create database responses for initial databases
        let databases: HashMap<String, GetDatabaseResponse> = initial_dbs
            ...
            .map(|name| {
                let response = GetDatabaseResponse::new(
                    ...
                    None,
                    HashMap::new(),
                    super::api_response::AuditRESTResponse::new(None, None, None, None, None),
                );
                (name, response)
            })
            .collect();
    ...
    pub fn add_database(&self, name: &str) {
        ...
        if !s.databases.contains_key(name) {
            ...
            s.databases.insert(name.to_string(), response);
        }
    }
    ...
    pub fn add_table(&self, database: &str, table: &str) {
        let mut s = self.inner.lock().unwrap();
        if !s.databases.contains_key(database) {
            ...
            s.databases.insert(database.to_string(), response);
        }
    ...
    /// Similar to Python's RESTCatalogServer config endpoint.
    /// Checks warehouse parameter and returns config if it matches.
    pub async fn get_config(
        Query(params): Query<HashMap<String, String>>,
        Extension(state): Extension<Arc<RESTServer>>,
    ) -> impl IntoResponse {
        // Check if warehouse parameter matches (similar to Python implementation)
    ...
    pub async fn create_database(
        ...
        if s.databases.contains_key(&name) {
            ...
            s.databases.insert(name, response);
    ...
    pub async fn drop_database(
        ...
        if s.databases.remove(&name).is_some() {
            ...
            s.no_permission_tables.retain(|key| !key.starts_with(&prefix));
            (StatusCode::OK, Json(serde_json::json!(""))).into_response()
        } else {
            let err = ErrorResponse::new(
                Some("database".to_string()),
...
impl MockServerHandle {
    ...
    pub fn add_no_permission_database(&self, name: &str) {
        self.state.add_no_permission_database(name);
    }

    pub fn add_table(&self, database: &str, table: &str) {
        self.state.add_table(database, table);
    }

    pub fn add_no_permission_table(&self, database: &str, table: &str) {
        self.state.add_no_permission_table(database, table);
    }
