...
const JDO_LIST_ITERATIVE: &str = "JDO_LIST_OPTS_IS_ITERATIVE";
const JDO_LIST_MARKER: &str = "JDO_LIST_OPTS_MARKER";
...
pub(crate) fn jindo_config_parse(mut props: HashMap<String, String>) -> Result<JindoStorageConfig> {
    ...
    if let Some(endpoint) = props.get(OSS_ENDPOINT).cloned() {
        if let Some((scheme, endpoint)) = endpoint.split_once("://") {
            if scheme.eq_ignore_ascii_case("http") || scheme.eq_ignore_ascii_case("https") {
                props.insert(OSS_ENDPOINT.to_string(), endpoint.to_string());
                props
                    .entry(OSS_HTTPS_ENABLE.to_string())
                    .or_insert_with(|| scheme.eq_ignore_ascii_case("https").to_string());
            }
        }
    }
...
impl JindoClient {
    ...
    fn list_page(
        ...
        self.with_handle(|handle| {
            ...
            self.set_list_option(options, JDO_LIST_ITERATIVE, "true")?;
            if let Some(marker) = marker.as_deref() {
                self.set_list_option(options, JDO_LIST_MARKER, marker)?;
            }

            let result = self.list_dir(handle, path.as_ptr(), recursive, options)?;
            if result.is_null() {
                return Err(OpendalError::new(
                    ErrorKind::Unexpected,
            ...
            let next_marker = if unsafe { (self.api.is_list_dir_result_truncated)(result) } {
    ...
    fn set_list_option(&self, options: JdoPtr, key: &str, value: &str) -> OpendalResult<()> {
        let key = to_cstring(key, "Jindo list option")?;
        let value = to_cstring(value, "Jindo list option")?;
        unsafe { (self.api.set_option)(options, key.as_ptr(), value.as_ptr()) };
        Ok(())
    }
...
mod tests {
    ...
    fn test_parse_config() {
        ...
        assert_eq!(
            config.properties.get(OSS_HTTPS_ENABLE).map(String::as_str),
            Some("true")
        );
        assert_eq!(
    ...
    #[test]
    fn test_parse_config_preserves_endpoint_protocol() {
        for (endpoint, configured, expected) in [
            ("http://localhost:9000", None, Some("false")),
            ("HTTPS://localhost:9000", None, Some("true")),
            ("https://localhost:9000", Some("false"), Some("false")),
            ("localhost:9000", Some("true"), Some("true")),
            ("localhost:9000", None, None),
        ] {
            let mut props = HashMap::from([(OSS_ENDPOINT.to_string(), endpoint.to_string())]);
            if let Some(configured) = configured {
                props.insert(OSS_HTTPS_ENABLE.to_string(), configured.to_string());
            }
            let config = jindo_config_parse(props).unwrap();
            assert_eq!(
                config.properties.get(OSS_ENDPOINT).map(String::as_str),
                Some(
                    endpoint
                        .split_once("://")
