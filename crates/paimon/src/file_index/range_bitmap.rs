...
impl RangeValue {
    ...
    fn equivalent_zero(&self) -> Option<Self> {
        match self {
            Self::Float(value) => match value.0 {
                0 => Some(Self::Float(JavaFloat(0x8000_0000))),
                0x8000_0000 => Some(Self::Float(JavaFloat(0))),
                _ => None,
            },
            Self::Double(value) => match value.0 {
                0 => Some(Self::Double(JavaDouble(0x8000_0000_0000_0000))),
                0x8000_0000_0000_0000 => Some(Self::Double(JavaDouble(0))),
                _ => None,
            },
            _ => None,
        }
    ...
    fn value(self, datum: &Datum) -> Result<RangeValue> {
        match (self, datum) {
            ...
            (Self::TimestampMillis, Datum::Timestamp { millis, nanos }) => {
                ...
                Ok(RangeValue::Timestamp(*millis))
            }
            ...
            (Self::LocalZonedTimestampMillis, Datum::LocalZonedTimestamp { millis, nanos }) => {
                validate_nanos(*nanos)?;
                Ok(RangeValue::LocalZonedTimestamp(*millis))
            }
...
impl RangeBitmapFileIndexReader {
    ...
    fn eq(&self, value: &RangeValue) -> RoaringBitmap {
        ...
        for value in std::iter::once(value.clone()).chain(value.equivalent_zero()) {
            if let Ok(code) = self.dictionary.binary_search(&value) {
                selected |= self.bsi.eq(code);
            }
        }
        selected
    }
    ...
    fn gt(&self, value: &RangeValue) -> RoaringBitmap {
        let equivalent = value.equivalent_zero();
...
mod tests {
    ...
    #[test]
    fn test_java_float_v1_preserves_rust_zero_and_nan_semantics() {
        ...
        for zero in [-0.0, 0.0] {
            assert_eq!(
                evaluate(PredicateOperator::Eq, &[Datum::Float(zero)]),
                selection([0, 1])
            );
            assert_eq!(
                evaluate(PredicateOperator::Lt, &[Datum::Float(zero)]),
                selection([])
            );
        }
        assert_eq!(
